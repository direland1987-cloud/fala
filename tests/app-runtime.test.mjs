...
test('hub boots with preserved lessons, front-page costs, and voice gated until setup', async () => {
  ...
  const ctx = vm.createContext({
    ...
    document,
    navigator: {},
    location: { hash: '', host: 'fala.example', protocol: 'https:' },
  ...
  const voice = fs
    .readFileSync(new URL('../public/voice.js', import.meta.url), 'utf8')
    .replace('export function createVoiceHub', 'function createVoiceHub');
  ...
  ctx.goto('voice');
  assert.match(elements.get('#app').innerHTML, /data-voice="start" disabled/);
  ctx.goto('costs');
  assert.ok(elements.get('#app').innerHTML.includes('No API lessons yet'));
  ctx.goto('lessons');
  assert.equal(ctx.currentNotebook().lessons.length, seed.lessons.length);
});
