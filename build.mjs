...
const assets = Object.fromEntries(
  fs
    .readdirSync('public')
    .filter((f) => fs.statSync('public/' + f).isFile())
    .map((f) => [
      '/' + f,
      {
        body: fs.readFileSync('public/' + f, 'utf8'),
        type: types[path.extname(f)] || 'text/plain',
      },
    ]),
);
// These modules are dependency-free ESM. Inline them without a framework migration.
const parts = ['public/seed.js', 'public/migrations.js', 'worker/domain.js', 'worker/index.js'].map(
  (f) =>
    fs
      .readFileSync(f, 'utf8')
      .replace(/^import .*;\n/gm, '')
