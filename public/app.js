...
function footer() {
  return `<footer class="footer"><span>${I('leaf')} Made for a little practice between conversations.</span><button class="btn text" data-action="data">${I('circleCheck')}<span data-sync-label>${voiceHub.syncLabel()}</span> · Backup & data</button><button class="btn text" data-nav="setup">${I('lock')}Connection</button><span class="subtle" data-app-version>v${esc(voiceHub.version)}</span></footer>`;
}
function page() {
  if (!voiceHub.ready)
